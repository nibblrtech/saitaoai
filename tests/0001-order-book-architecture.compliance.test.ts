...
describe("0001-order-book-architecture: Generated Non-ArchUnit Compliance Constraints", () => {
  ...
  // ADR_MAPPING_RULE: compliance-order-book-queue-position
  it("should provide resting-order queue position preservation evidence for this constraint", () => {
    const candidateRoots = ["src/order-book"];
    const existingRoots = candidateRoots.filter((root) => existsSync(root));
    ...
    const requiredTerms: string[] = ["queue", "cancel", "replace"];
    const missingTerms = requiredTerms.filter((term) => !corpus.includes(term));
  ...
  // ADR_MAPPING_RULE: compliance-order-book-not-crossed
  it("should provide crossed-book prevention evidence for this constraint", () => {
    const candidateRoots = ["src/order-book"];
    const existingRoots = candidateRoots.filter((root) => existsSync(root));
    ...
    const requiredTerms: string[] = ["crossed", "bid", "ask"];
    const missingTerms = requiredTerms.filter((term) => !corpus.includes(term));
  ...
  it("should provide versioned reference-data snapshot evidence for this constraint", () => {
    const candidateRoots = ["src/order-book"];
    const existingRoots = candidateRoots.filter((root) => existsSync(root));
    ...
    const requiredTerms: string[] = ["snapshot", "immutable", "version"];
    const missingTerms = requiredTerms.filter((term) => !corpus.includes(term));
  ...
  // ADR_MAPPING_RULE: compliance-order-book-duplicate-order-ids
  it("should provide duplicate order identifier prevention evidence for this constraint", () => {
    const candidateRoots = ["src/order-book"];
    const existingRoots = candidateRoots.filter((root) => existsSync(root));
